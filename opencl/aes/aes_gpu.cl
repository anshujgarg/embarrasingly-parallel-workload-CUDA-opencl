...
__constant uint8_t RC_gpu[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

/*
 * Sbox
 */
__constant uint8_t S_gpu[256] = {
...
static void shift_rows_local(uint8_t *state) {
    uint8_t temp;
    // row1
    temp        = *(state+1);
    *(state+1)  = *(state+5);
...
static void shift_rows(__global uint8_t *state) {
    uint8_t temp;
    // row1
    temp        = *(state+1);
    *(state+1)  = *(state+5);
...
void aes_encrypt_128_gpu(__global uint8_t *roundkeys, __global uint8_t *plaintext, __global uint8_t *ciphertext) {
...
__kernel void aes_encrypt_gpu(__global uint8_t *plaintext,__global uint8_t *ciphertext,__global uint8_t *roundkeys,unsigned int total_blocks) {
